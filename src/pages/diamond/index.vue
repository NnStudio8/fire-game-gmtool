...
const diamond = withStorageRef('2066f8d3948b', 1000)

const onSubmit = async (mode?: '+' | '-') => {
  const check = {
    请输入服务器ID: !!worldid.value,
    请输入角色ID: !!roleid.value,
    请输入钻石数量: !!diamond.value,
  }
  ...
  if (mode && mode === '-') {
    data.gm = `submoney 3 ${diamond.value}`
  } else {
    data.gm = `adddiamond ${diamond.value}`
  }
...
<template>
  <div class="index">
    <div class="center">
      ...
      <!-- 钻石数量 -->
      <div class="form-item">
        <div class="form-item-title">钻石数量</div>
        <div class="form-item-value">
          <el-input-number class="form-input-50" v-model="diamond" :min="1" :max="1000000" />
        </div>
      </div>

      <!-- 提交表单 -->
      <div class="submit">
        <el-button type="primary" @click="onSubmit('+')">增加钻石</el-button>
        <el-button type="danger" @click="onSubmit('-')">减少钻石</el-button>
      </div>
    </div>
  </div>
</template>
